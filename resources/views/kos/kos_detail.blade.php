...
<div class="main-content">
  <div class="detail-container">
    <h2>{{ $kosan->nama_kos ?? 'Nama tidak tersedia' }}</h2>
    <p><strong>Kategori:</strong> {{ $kosan->kategori_kos ?? '-' }}</p>
    <p><strong>Lokasi:</strong> {{ $kosan->lokasi ?? '-' }}</p>
    <p><strong>Alamat Lengkap:</strong> {{ $kosan->alamat ?? '-' }}</p>
    <p><strong>Harga:</strong> Rp{{ isset($kosan->harga) ? number_format($kosan->harga,0,',','.') : '-' }}</p>
    <p><strong>Nomor yang bisa dihubungi:</strong> {{ $kosan->no_telepon ?? '-' }}</p>
    <p><strong>Foto Kos:</strong></p>
    ...
    <div class="image-preview">
      ...
      @for($i=1; $i<=6; $i++)
        @php $f = 'foto'.$i; @endphp
        @if(!empty($kosan->$f))
          @php $url = asset('storage/uploads/kosan/' . $kosan->$f); $images[] = $url; @endphp
          <img src="{{ $url }}" data-index="{{ count($images)-1 }}" alt="Foto Kos {{ $i }}">
        @endif
      @endfor
    </div>

    <div class="tombol-wrapper">
      @if($kosan->no_telepon && Auth::id() != $kosan->id_user)
        <a href="{{ $kosan->whatsapp_link }}" class="btn-primary" target="_blank">Hubungi sekarang</a>
      @elseif(Auth::id()==$kosan->id_user)
        <span class="btn-primary disabled">Ini kos Anda</span>
      @else
        <span class="btn-primary disabled">Nomor tidak tersedia</span>
      @endif
      <a href="{{ route('kos.market') }}" class="btn-primary">Kembali ke Eksplore Kost</a>
      @auth
        @if(Auth::id()==$kosan->id_user)
          <a href="{{ route('kos.edit',$kosan->id_kos) }}" class="btn-primary">Edit Kos</a>
          <form action="{{ route('kos.delete',$kosan->id_kos) }}" method="POST" style="display:inline">
            @csrf @method('DELETE')
            <button type="submit" class="btn-primary" onclick="return confirm('Yakin menghapus kos ini?')">Hapus Kos</button>
          </form>
        @endif
      @endauth
    </div>
...
<footer>
  © {{ date('Y') }} CuanManis Kos. Semua Hak Dilindungi 🍊
</footer>
